    ...
    <>
      <header className={`fixed top-0 left-0 right-0 z-30 border-b border-white/15 bg-black/90 backdrop-blur-xl transition-transform duration-300 ${navVisible ? "translate-y-0" : "-translate-y-full"}`}>
        <nav className="mx-auto flex w-full max-w-6xl items-center justify-between px-4 py-3 sm:px-6 sm:py-4">
          ...
          <a href="/" className="flex items-center gap-2.5" onClick={() => setMenuOpen(false)} aria-label="PhonesAI home">
            <div className="relative flex items-center justify-center">
              <div className="absolute inset-0 rounded-full bg-blue-500/25 blur-md scale-150" aria-hidden="true" />
              <Image src="/logo.png" alt="PhonesAI logo" width={36} height={36} className="relative h-8 w-auto sm:h-9" priority />
            </div>
          ...
          <div className="hidden md:flex items-center gap-6 text-sm font-medium text-white/80">
            ...
            <button onClick={openUstaadJi} className="transition hover:text-white" aria-label="Open Ustaad Ji AI assistant">Ustaad Ji</button>
            <a href="https://wa.me/923200801010" target="_blank" rel="noopener noreferrer" className="transition hover:text-white">Contact</a>
            <button onClick={() => setCartOpen(true)} aria-label={`Open cart${count > 0 ? `, ${count} items` : ""}`}
              ...
              <span>Cart</span>
              {count > 0 && (
                <span className="absolute -right-1.5 -top-1.5 flex h-5 w-5 items-center justify-center rounded-full bg-blue-500 text-[10px] font-bold text-white" aria-hidden="true">{count}</span>
              )}
            </button>
          </div>
          ...
          <div className="flex md:hidden items-center gap-2">
            <button onClick={() => setCartOpen(true)} aria-label={`Open cart${count > 0 ? `, ${count} items` : ""}`}
              ...
              {count > 0 && (
                <span className="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-blue-500 text-[9px] font-bold text-white" aria-hidden="true">{count}</span>
              )}
            </button>
            <button onClick={() => setMenuOpen(!menuOpen)} aria-label={menuOpen ? "Close menu" : "Open menu"}
              ...
              <span className={`h-0.5 w-4 rounded-full bg-white transition-all duration-200 ${menuOpen ? "translate-y-2 rotate-45" : ""}`} aria-hidden="true" />
              <span className={`h-0.5 w-4 rounded-full bg-white transition-all duration-200 ${menuOpen ? "opacity-0" : ""}`} aria-hidden="true" />
              <span className={`h-0.5 w-4 rounded-full bg-white transition-all duration-200 ${menuOpen ? "-translate-y-2 -rotate-45" : ""}`} aria-hidden="true" />
            </button>
          </div>
        </nav>
        ...
        {menuOpen && (
          <div className="md:hidden border-t border-white/10 bg-black/95 px-4 py-4 space-y-1">
            ...
            <button onClick={() => { openUstaadJi(); setMenuOpen(false); }} aria-label="Open Ustaad Ji AI assistant" className="flex w-full items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium text-white/80 transition hover:bg-white/5 hover:text-white">🤖 Ustaad Ji</button>
            <a href="https://wa.me/923200801010" target="_blank" rel="noopener noreferrer" onClick={() => setMenuOpen(false)} className="flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium text-white/80 transition hover:bg-white/5 hover:text-white">💬 Contact on WhatsApp</a>
            <div className="pt-2 border-t border-white/10">
              <button onClick={() => { setCartOpen(true); setMenuOpen(false); }} aria-label="View cart" className="flex w-full items-center justify-between rounded-xl bg-blue-500/10 border border-blue-400/20 px-4 py-3 text-sm font-semibold text-blue-300">
                <span>🛒 View Cart</span>
      ...
      <div className={`fixed top-0 right-0 z-50 h-full w-full max-w-md flex flex-col bg-[#0a0a0a] border-l border-white/10 shadow-2xl transition-transform duration-300 ${cartOpen ? "translate-x-0" : "translate-x-full"}`}
        ...
        <div className="flex items-center justify-between border-b border-white/10 px-5 py-4 shrink-0">
          <div className="flex items-center gap-3">
            ...
            {count > 0 && <span className="rounded-full border border-blue-400/30 bg-blue-500/10 px-2.5 py-0.5 text-xs text-blue-300">{count} item{count !== 1 ? "s" : ""}</span>}
          </div>
          <button onClick={() => setCartOpen(false)} aria-label="Close cart" className="flex h-8 w-8 items-center justify-center rounded-full border border-white/15 text-white/60 transition hover:text-white">✕</button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center py-20">
              ...
              <p className="text-white/60 font-medium">Cart is empty</p>
              <p className="text-xs text-white/40 mt-1">Add phones from the shop</p>
              <button onClick={() => { setCartOpen(false); window.location.href = "/shop"; }} className="mt-6 rounded-xl border border-white/15 px-6 py-2.5 text-sm text-white/60 transition hover:text-white">Browse Shop →</button>
            </div>
          ) : (
            items.map((item) => (
              <div key={item.id} className="flex gap-3 rounded-2xl border border-white/10 bg-white/[0.02] p-3.5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-white/60 mt-0.5">{item.storage} • {item.color}</p>
                  <p className="mt-1.5 text-sm font-bold text-white">Rs. {(item.discount_price ?? item.price).toLocaleString()}</p>
                </div>
                <button onClick={() => removeItem(item.id)} aria-label={`Remove ${item.model} from cart`} className="shrink-0 self-start rounded-lg border border-red-500/20 p-1.5 text-red-400 transition hover:bg-red-500/10">
        ...
        {items.length > 0 && (
          <div className="shrink-0 border-t border-white/10 px-5 py-4 space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-sm text-white/60">Total ({count} items)</span>
            ...
            <a href="/checkout?cart=true" onClick={() => setCartOpen(false)} className="block w-full rounded-2xl bg-blue-500 py-4 text-center text-sm font-bold text-white transition hover:bg-blue-400">
              Checkout → Rs. {total.toLocaleString()}
            </a>
            <button onClick={clearCart} aria-label="Clear all items from cart" className="w-full text-center text-xs text-white/40 transition hover:text-white/60 py-1">Clear cart</button>
          </div>
        )}
      </div>
    </>
