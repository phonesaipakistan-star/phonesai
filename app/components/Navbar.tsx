...
export default function Navbar() {
  ...
  return (
    <>
      ...
      {/* Cart Backdrop */}
      {cartOpen && <div className="fixed inset-0 z-40 bg-black/60 backdrop-blur-sm" onClick={() => setCartOpen(false)} />}
      ...
      <div className={`fixed top-0 right-0 z-50 h-full w-full max-w-md flex flex-col bg-[#0a0a0a] border-l border-white/10 shadow-2xl transition-transform duration-300 ${cartOpen ? "translate-x-0" : "translate-x-full"}`}>
        <div className="flex items-center justify-between border-b border-white/10 px-5 py-4 shrink-0">
          ...
          <button onClick={() => setCartOpen(false)} className="flex h-8 w-8 items-center justify-center rounded-full border border-white/15 text-white/50 transition hover:text-white">✕</button>
        </div>
          ...
          {items.length === 0 ? (
            ...
            items.map((item) => (
              <div key={item.id} className="flex gap-3 rounded-2xl border border-white/10 bg-white/[0.02] p-3.5">
                ...
                <button onClick={() => removeItem(item.id)} className="shrink-0 self-start rounded-lg border border-red-500/20 p-1.5 text-red-400 transition hover:bg-red-500/10">
        ...
        {items.length > 0 && (
          <div className="shrink-0 border-t border-white/10 px-5 py-4 space-y-3">
            ...
            <button onClick={clearCart} className="w-full text-center text-xs text-white/25 transition hover:text-white/50 py-1">Clear cart</button>
          </div>
        )}
      </div>
    </>
