...
export default function Navbar() {
  ...
  return (
    <>
      {/* Navbar */}
      <header className={`fixed top-0 left-0 right-0 z-30 border-b border-white/15 bg-black/80 backdrop-blur-xl transition-transform duration-300 ${navVisible ? "translate-y-0" : "-translate-y-full"}`}>
            ...
            items.map((item) => (
              <div key={item.id} className="flex gap-4 rounded-2xl border border-white/10 bg-white/[0.02] p-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="mt-2 text-sm font-bold text-white">Rs. {(item.discount_price ?? item.price).toLocaleString()}</p>
                </div>
